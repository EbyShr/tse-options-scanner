    ...
    def _process_pool(df_subset: pd.DataFrame, category_label: str):
        ...
        for rank, (_, row) in enumerate(df_subset.iterrows(), 1):
            ...
            records.append({
                ...
                "امتیاز نقدینگی ترکیبی (۳۰٪)": round(float(row.get("امتیاز نقدینگی ترکیبی", 0.0)), 1),
                "امتیاز نقدینگی ساختاری (۱۸٪)": round(float(row.get("امتیاز نقدینگی ساختاری", 0.0)), 1),
                "امتیاز جهش لحظه‌ای (۱۲٪)": round(float(row.get("امتیاز جهش لحظه‌ای", 0.0)), 1),
                "امتیاز ارزش نسبی حباب (۳۰٪)": round(float(row.get("امتیاز ارزش نسبی (حباب)", 0.0)), 1),
